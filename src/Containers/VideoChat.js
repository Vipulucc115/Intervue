...
class VideoChat extends Component {
    ...
    videoSocket.addEventListener("message", (event) => {
      ...
      if (on["offerMade"]) {
        ...
        pc.setRemoteDescription(
          new RTCSessionDescription(on["offerMade"].offer)
        )
          .then(() => {
        ...
        pc.setRemoteDescription(
          new RTCSessionDescription(on["answerMade"].answer)
        )
          .then(() => {
            console.log("remote description set");
        ...
        on["candidate"].map((candidate) =>
          VideoHelper.addIceCandidate(candidate)
        );
  ...
  componentWillUnmount() {
    if (this.localRef.current.srcObject)
      this.localRef.current.srcObject
        .getTracks()
    ...
    this.state.pc
      ...
      .then((sdp) => {
        this.state.pc
          ...
          .then(() => {
            this.state.videoSocket.send(
              JSON.stringify({ makeAnswer: { answer: sdp } })
            );
            this.localRef.current.srcObject = localMediaStream;
            this.setState({ peerConnected: true });
          })
  ...
  toggleVideo = () => {
    ...
    this.localRef.current.srcObject.getVideoTracks()[0].enabled =
      !controls.video;
    this.setState({
      controls: { video: !controls.video, audio: controls.audio },
    });
  };

  toggleAudio = () => {
    const controls = { ...this.state.controls };
    this.localRef.current.srcObject.getAudioTracks()[0].enabled =
      !controls.audio;
    this.setState({
      controls: { ...this.state.controls, audio: !controls.audio },
    });
  };
